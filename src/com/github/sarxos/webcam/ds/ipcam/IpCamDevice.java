...
public class IpCamDevice implements WebcamDevice {
			...
			try {
				while (running) {
					...
					try {
						...
						if (failOnError) {
							throw new WebcamException("Cannot read MJPEG frame", e);
						}
					}
				}
			} catch (Exception ex) {
				
				// wrap the exception if necessary
				if (ex instanceof WebcamException) {
					exception = (WebcamException)ex;
				} else {
					exception = new WebcamException(ex);
				}
				throw exception;
			} finally {
				// unlock any other threads who are waiting on this one
				synchronized (lock) {
					lock.notifyAll();
				}
			}
	...
	public  IpCamHttpClient client = new IpCamHttpClient();  // nasty hack to allow other stuff
